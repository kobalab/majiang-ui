...
module.exports = class PaipuFile {
    ...
    constructor(root, storage, callback, url, hash) {
        this._root    = root;
        this._row     = $('.row', root);
        ...
        $('form').on('submit', (ev)=>{
            ...
            let id = url.replace(/^.*\?log=/,'')
                        ...
                        .replace(/^.*\//,'')
                        .replace(/\..*$/,'');
            location = '?' + tenhou_log + id + '.json' + hash;
            return false;
        });
    ...
    redraw() {
        ...
        if (this.isEmpty) {
            hide($('.download', this._root));
            if ($('input[name="storage"]', this._root).prop('checked'))
                    hide($('form', this._root));
            else    show($('form', this._root));
        }
        else {
            show($('.download', this._root));
            hide($('form', this._root));
        }
