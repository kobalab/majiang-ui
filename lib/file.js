...
module.exports = class PaipuFile {
    ...
    constructor(root, storage, viewer, stat, base, url, hash) {
        this._root    = root;
        this._row     = $('.row', root);
        ...
        let base_url = location.href.replace(/\?.*$/,'').replace(/[^\/]*$/,'');
        let otigin   = location.origin;
        if (base.substr(0, base_url.length) == base_url) {
            this._base = base.substr(base_url.length);
        }
        else if (base.substr(0, origin.length) == origin) {
            this._base = base.substr(origin.length);
        }
        else {
            this._base = base;
        }
        ...
        $('form').on('submit', (ev)=>{
            ...
            let id = url.replace(/^.*\?log=/,'')
                        ...
                        .replace(/^.*\//,'')
                        .replace(/\..*$/,'');
            location = '?' + this._base + id + '.json' + hash;
            return false;
        });
