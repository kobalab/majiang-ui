...
module.exports = class Paipu {
    ...
    set_handler() {
        ...
        $('.last',     ctrl).on('click', ()=>this.forward());
        $('.first',    ctrl).on('click', ()=>this.backward());
        $('.summary',  ctrl).on('click', ()=>this.summary());
        $('.sound',    ctrl).on('click', ()=>this.sound(! this._view.sound_on));
        $('.minus',    ctrl).on('click', ()=>this.speed(this._speed - 1));
        $('.plus',     ctrl).on('click', ()=>this.speed(this._speed + 1));
        $('.exit',     ctrl).on('click', ()=>this.exit());
        $('> .shoupai', this._root).on('click', '.pai', ()=>this.shoupai());
        $('> .he',      this._root).on('click', '.pai', ()=>this.he());
        for (let i = 0; i < 4; i++) {
            $(`.player.${class_name[i]}`, this._root)
                            .on('click', ()=>this.viewpoint(
