...
function remake_defen(hule, rule) {
    ...
    if (hule.hupai.find(h => h.fanshu[0] == '*')) {
        ...
        if (! rule['ダブル役満あり'])
                hule.hupai.forEach(h => h.fanshu = h.fanshu[0]);
        hule.damanguan = ! rule['役満の複合あり'] ? 1
                       : hule.hupai.map(h => h.fanshu.length)
                                   .reduce((x , y)=> x + y);
...
module.exports = class PaipuEditor {
    ...
    init_hule_dialog() {
        ...
        $('.hupai, [name="fu"]').off('change').on('change', ()=>{
