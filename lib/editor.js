...
function paiin(node) {
    $('.paiin > .input', node).hide();
    $('.paiin', node).on('focus', (ev)=>{
        let tabindex = $(ev.target).attr('tabindex');
        $(ev.target).removeAttr('tabindex');
        $('> .pai', $(ev.target)).hide();
        $('> .input', $(ev.target)).show();
        $('input', $(ev.target)).attr('tabindex', tabindex);
        $('input', $(ev.target)).trigger('focus');
    });
    $('.paiin input', node).on('blur', (ev)=>{
        let tabindex = $(ev.target).attr('tabindex');
        $(ev.target).removeAttr('tabindex');
        $(ev.target).parent('.input').hide();
        $(ev.target).parent('.input').parent('.paiin').attr('tabindex', tabindex);
        $('>.pai', $(ev.target).parent('.input').parent('.paiin')).show();
    });
}
