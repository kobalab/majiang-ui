...
function draw_qipai(root, pai, paistr, badpai) {
    let col = root.children().eq(0).empty();
    root.empty();
    let n = 0;
    for (let p of split_pai(Majiang.Shoupai.fromString(paistr).toString())) {
...
function count_pai(log) {
    ...
    function decrease(p) {
        let s = p[0], n = +p[1]||5;
        paishu[s][n]--;
        if (paishu[s][n] < 0) rv[p] = true;
    }

    for (let data of log) {
        if (data.qipai) {
            for (let l = 0; l < 4; l++) {
                for (let p of split_pai(data.qipai.shoupai[l])) {
                    decrease(p);
                }
            }
...
function remake_defen(hule, rule) {
    ...
    if (hule.hupai.find(h => h.fanshu[0] == '*')) {
        delete hule.fu;
        delete hule.fanshu;
        hule.hupai = hule.hupai.filter(h => h.name && h.fanshu.match(/^\**$/));
        hule.damanguan = Math.min(hule.hupai.map(h => h.fanshu.length)
                                            .reduce((x , y)=> x + y),
                                  hule.damanguan);
...
module.exports = class PaipuEditor {
    ...
    draw_score() {
        ...
        function draw_baopai(i, p, val, error) {

            $('[name="baopai"]', score).eq(i).val(val||'');
            $('.flip', score).eq(i).attr('tabindex', 0);

            if (! val) {
                $('.flip >.pai', score).eq(i).empty().append(pai('_'));
            }
            else if (Majiang.Shoupai.valid_pai(p)) {
                $('.flip >.pai', score).eq(i).empty().append(pai(p));
            }
            else {
                $('.flip', score).eq(i).addClass('error');
                $('.flip >.pai', score).eq(i).empty().text(val);
            }
            if (error) {
                $('.flip', score).eq(i).addClass('error');
            }
        ...
        $('.flip', score).attr('tabindex', -1).removeClass('error');
        draw_baopai(0, qipai.baopai, qipai.baopai);
        let baopai = [], n = 1;
        for (let data of log) {
            if (data.kaigang) {
                draw_baopai(n, data.kaigang.baopai,
    ...
    draw_paipu() {
        ...
        for (let i = 0; i < log.length; i++) {
            ...
            if (data.qipai) {
                ...
                l  = data.zimo.l;
                md = 'm';
                error = error || badpai[data.zimo.p.slice(0,2)];
                ...
                l  = data.gangzimo.l;
                md = 'm';
                error = error || badpai[data.gangzimo.p.slice(0,2)];
