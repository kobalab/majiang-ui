...
module.exports = class PaipuEditor {
    ...
    select_log(log_idx) {
        ...
        hide($('form.hule', this._root));
        hide($('form.pingju', this._root));
    ...
    make_hule(l, fubaopai = []) {
        ...
        let shoupai = this._model.shoupai[l].clone();
        let rongpai, baojia;
        if (model.lunban != l) {
            let dapai = get_last_dapai(log);
            if (! dapai) return;
            if (dapai.l == l) return;
            baojia  = dapai.l;
            rongpai = dapai.p.slice(0,2)
                        + ['_','+','=','-'][(4 + baojia - l) % 4];
        }
        let param = {};
        param.zhuangfeng = model.zhuangfeng;
        param.menfeng    = l;
        param.lizhi      = shoupai.lizhi && 1;
        param.baopai     = model.shan.baopai;
        param.fubaopai   = fubaopai;
        param.changbang  = model.changbang;
        param.lizhibang  = model.lizhibang;
        ...
        return {
            ...
            baojia:     baojia,
            fubaopai:   shoupai.lizhi ? fubaopai : null,
            fu:         hule.fu,
            fanshu:     hule.fanshu,
    ...
    init_hule_dialog() {
        ...
        const form      = $('form.hule', this._root);
