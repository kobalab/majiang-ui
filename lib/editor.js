...
module.exports = class PaipuEditor {
    ...
    select_log(log_idx) {
        ...
        if (log_idx >= 0) {
            ...
            $('.menu select[name="game"]')
                                .val(qipai.zhuangfeng * 4 + qipai.jushu);
            $('.menu select[name="game"]').off('change').on('change', ()=>{
                qipai.zhuangfeng = $('.menu select[name="game"]').val() >> 2;
                qipai.jushu      = $('.menu select[name="game"]').val() % 4;
                this.select_log(this._log_idx);
            })

            $('.menu input[name="changbang"]').val(qipai.changbang);
            $('.menu input[name="changbang"]').off('change').on('change', ()=>{
                qipai.changbang = + $('.menu input[name="changbang"]').val();
                this.select_log(this._log_idx);
            });

            $('.menu input[name="lizhibang"]').val(qipai.lizhibang);
            $('.menu input[name="lizhibang"]').off('change').on('change', ()=>{
                qipai.lizhibang = + $('.menu input[name="lizhibang"]').val();
            });

            $('.menu input[name="baopai"]').eq(0).val(qipai.baopai);
            $('.menu input[name="baopai"]').off('change').on('change', ()=>{
                qipai.baopai = $('.menu input[name="baopai"]').eq(0).val();
            });
