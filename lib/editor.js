...
function moda_accessor(root) {
    return function(t, l, md, val) {
        const moda = (md == 'm')
                    ? $('input', $('.mo', root).eq(l)).eq(t)
                    : $('input', $('.da', root).eq(l)).eq(t);
        return val == null ? moda.val() : moda.val(val);
    }
}
...
module.exports = class PaipuEditor {
    ...
    init_moda() {
        ...
        let mo = $('.paipu .mo input', this._root).eq(0).val('');
        for (let l = 0; l < 4; l++) {
            $('.paipu .mo').eq(l).empty();
            for (let t = 1; t <= 30; t++) {
                let r = mo.clone().attr('tabindex', t);
                $('.paipu .mo').eq(l).append(r)
            }
        }
        let da = $('.paipu .da input', this._root).eq(0).val('');
        for (let l = 0; l < 4; l++) {
            $('.paipu .da').eq(l).empty();
            for (let t = 1; t <= 30; t++) {
                let r = da.clone().attr('tabindex', t);
    ...
    draw_paipu() {
        ...
        for (let data of log) {
            ...
            if (data.qipai) {
                ...
                if (data.zimo.l < l) t++;
                l = data.zimo.l;
                moda(t, l, 'm', data.zimo.p);
            }
            else if (data.dapai) {
                if (data.dapai.l < l) t++;
                l = data.dapai.l;
                moda(t, l, 'd', data.dapai.p[2] == '_'
                                        ? data.dapai.p.slice(2) : data.dapai.p);
            }
            else if (data.fulou) {
                t++;
                l = data.fulou.l;
                moda(t, l, 'm', data.fulou.m);
            }
            else if (data.gang) {
                moda(t, l, 'd', data.gang.m);
            }
