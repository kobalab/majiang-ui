...
function get_last_defen(log) {

    let defen, lizhi;

    for (let data of log) {

        if (lizhi != null && ! (data.hule || data.pingju)) {
            defen[lizhi] += -1000;
            lizhi = null;
        }

        if (data.qipai) {
            defen = data.qipai.defen.concat();
        }
        else if (data.dapai && data.dapai.p.slice(-1) == '*') {
            lizhi = data.dapai.l;
        }
        else if (data.hule || data.pingju) {
...
module.exports = class PaipuEditor {
    ...
    add_log(log_idx) {
        ...
        let defen = log_idx >= 0 ? get_last_defen(this._paipu.log[log_idx])
                                 : [0,0,0,0].map(x => this._rule['配給原点']);
        let qipai = {
            zhuangfeng: 0,
            jushu:      0,
            changbang:  0,
