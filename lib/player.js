...
module.exports = class Player extends Majiang.Player {
    ...
    set_timer(limit, allowed = 0) {
        ...
        this._timer_id = setInterval(()=>{
            ...
            let time = 1 + ((time_limit - Date.now())/1000)|0;
            if (time <= limit || time <= allowed) this._node.timer.text(time);

            if (allowed && time < this._allowed_time) this._allowed_time = time;
        }, 100);
    }
    ...
    action(msg, callback) {
        ...
        let limit = 5, allowed;
        if      (msg.qipai)             this._allowed_time = 10;
        else if (msg.kaiju || msg.hule || msg.pingju)
                                        limit = 10;
        else if (msg.zimo || msg.fulou) allowed = this._allowed_time;
        else if (msg.jieju)             limit = 0;
        // console.log('allowed:', this._allowed_time);
